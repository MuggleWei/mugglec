...
## v2.6.1
* 修复 `muggle_memory_resource_t` 在较老的 MSVC 编译器上的编译错误

## v2.6.0
* 新增 `muggle_memory_resource_t`
* 更新 `muggle_ts_memory_pool_t`, 支持使用 `muggle_memory_resource_t` 来初始化
* 更新 `muggle_channel_t`, 支持使用 `muggle_memory_resource_t` 来初始化
* 更新 `muggle_shm_t`, 初始化支持指定使用大页标志

## v2.5.1
* 修复 `muggle_synclock_lock` 中错误的处理 CAS 虚假失败的问题

## v2.5.0
* 新增 `muggle_socket_evloop_pipe_read_n` 以支持 `evloop_pipe` 批量读取

## v2.4.2
* 修复 `muggle_memory_pool_t`/`muggle_ts_memory_pool_t` 初始化超过 4GB 时会导致的问题
