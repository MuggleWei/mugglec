...
TEST(log, fmt)
{
	...
	num_write = muggle_log_fmt_gen(fmt_flag, &arg, "hello", buf, sizeof(buf));
	EXPECT_GT(num_write, 0);
	EXPECT_STREQ(buf, "<F>log_fmt_test_file| - hello");

	fmt_flag = MUGGLE_LOG_FMT_LINE;
	num_write = muggle_log_fmt_gen(fmt_flag, &arg, "hello", buf, sizeof(buf));
	EXPECT_GT(num_write, 0);
	EXPECT_STREQ(buf, "<l>666| - hello");
	...
	num_write = muggle_log_fmt_gen(fmt_flag, &arg, "hello", buf, sizeof(buf));
	EXPECT_GT(num_write, 0);
	EXPECT_TRUE(muggle_str_startswith(buf, "<L>WARNING|<F>log_fmt_test_file|<l>666|<f>log_fmt_test_func|<T>"));
	EXPECT_TRUE(muggle_str_endswith(buf, "| - hello"));
}
