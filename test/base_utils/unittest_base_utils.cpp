...
{
	EXPECT_TRUE(IS_POW_OF_2(2));
	EXPECT_FALSE(IS_POW_OF_2(3));
	EXPECT_TRUE(IS_POW_OF_2(4));
	EXPECT_FALSE(IS_POW_OF_2(5));
	EXPECT_FALSE(IS_POW_OF_2(6));
	EXPECT_FALSE(IS_POW_OF_2(7));
	EXPECT_TRUE(IS_POW_OF_2(8));

	for (size_t i = 0; i < sizeof(po2_arr) / sizeof(po2_arr[0]); ++i)
	{
		EXPECT_TRUE(IS_POW_OF_2(po2_arr[i]));
		// for save time, only test i < 16
		if (i < 16)
		{
			for (uint32_t j = po2_arr[i]+1; j < po2_arr[i+1]; ++j)
			{
				EXPECT_FALSE(IS_POW_OF_2(j));
			}
		}
	}
}
...
{
	EXPECT_EQ(ROUND_UP_POW_OF_2_MUL(3, 2), 4);
	EXPECT_EQ(ROUND_UP_POW_OF_2_MUL(3, 4), 4);
	EXPECT_EQ(ROUND_UP_POW_OF_2_MUL(3, 8), 8);

	EXPECT_EQ(ROUND_UP_POW_OF_2_MUL(9, 2), 10);
	EXPECT_EQ(ROUND_UP_POW_OF_2_MUL(9, 4), 12);
	EXPECT_EQ(ROUND_UP_POW_OF_2_MUL(9, 8), 16);
...
TEST(baseutils, next_pow_of_2)
{
	...
	for (uint32_t i = 1; i < 16; ++i)
	{
		...
		{
			uint32_t x = (uint32_t)next_pow_of_2((uint64_t)j);
			EXPECT_EQ(x, upper);
		}
	}
}
