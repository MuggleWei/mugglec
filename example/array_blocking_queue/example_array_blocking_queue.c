...
muggle_thread_ret_t consumer_thread(void *void_arg)
{
	...
	while (1)
	{
		muggle_benchmark_block_t *block = (muggle_benchmark_block_t*)muggle_array_blocking_queue_take(arg->queue);
		if (block == NULL)
		{
			break;
		}
...
muggle_thread_ret_t producer_thread(void *void_arg)
{
	...
	while (1)
	{
		...
		if (idx < arg->total)
		{
			...
			muggle_array_blocking_queue_put(arg->queue, &arg->blocks[idx]);
...
void producer_consumer(int capacity, int total, int cnt_producer, int cnt_consumer, int cnt_interval, int interval_ms)
{
	muggle_array_blocking_queue_t queue;
	...
	for (int i = 0; i < cnt_consumer; ++i)
	{
		muggle_array_blocking_queue_put(&queue, NULL);
	}
...
int main()
{
	...
	producer_consumer(capacity, total, hc, 1, cnt_interval, interval_ms);

	// 1 producer, mul consumer
	producer_consumer(capacity, total, 1, hc, cnt_interval, interval_ms);
