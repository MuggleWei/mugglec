...
int main(int argc, char *argv[])
{
	...
	// worker thread
	muggle_channel_t chan_arr[NUM_WORKER];
	worker_thread_args_t worker_args[NUM_WORKER];
	for (int i = 0; i < NUM_WORKER; i++)
	{
		worker_args[i].idx = i;
		worker_args[i].chan = &chan_arr[i];
		muggle_channel_init(worker_args[i].chan, 128, 0);

		muggle_thread_t th;
		muggle_thread_create(&th, worker_thread, (void*)&worker_args[i]);
	...
	ev_data.user_data = (void*)chan_arr;
