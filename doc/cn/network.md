...
可以看到, 经过简单的替换, 我们已经可以轻松的同时处理多个连接上来的客户端了, 并且CPU资源占用也不高. 所以这就完成了吗? 哈哈哈, 当然没有, 当连接数量少时, 这种方案似乎没有问题, 但是当连接数量成百上千时, 为每个连接创建一个单独的线程的开销便显得过于昂贵了.  
...
Windows还提供专属的异步I/O: IOCP(IO Completion Ports) API.  
...
## 使用mugglec网络模块
如果用户想要专注于自己应用层的协议, 而避免把精力过多的分散到底层的细节中去, 可以使用现成的网络库. 不同的语言都有十分类似的优秀的网络库存在, 比如c有libevent, libuv, Java有netty; 这些网络库都是知名且有众多的用户, 也十分容易上手  
当然, mugglec中也带有自己的网络模块, 可以直接使用  
