								 ...
								 muggle_sync_t data_size)
{
	...
	pool->block_size =
		data_size + next_pow_of_2(sizeof(muggle_ring_mpool_block_head_t));
	pool->alloc_idx = 0;
...
void *muggle_ring_memory_pool_alloc(muggle_ring_memory_pool_t *pool)
{
	...
	muggle_atomic_int32 expected = 0;
	do {
		...
		expected = 0;
	} while (!muggle_atomic_cmp_exch_strong(&block->in_use, &expected, 1,
